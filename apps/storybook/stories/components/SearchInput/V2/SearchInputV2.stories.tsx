...
- **States**: \`disabled\`, \`error\` (boolean), native \`required\` / \`aria-required\`
- **Theming**: \`SEARCH_INPUT_V2\` tokens (light / dark via ThemeProvider)

## Accessibility
- Native text input with \`role="searchbox"\`
- \`aria-invalid\` when \`error\` is true; \`aria-required\` when \`required\`
...
export const VisualStates: Story = {
    ...
    parameters: {
        ...
        docs: {
            description: {
                story: 'Static snapshot of common states for Chromatic and design review: empty, value, required, error, disabled, built-in clear.',
            },
        },
...
export const Accessibility: Story = {
    ...
    render: function AccessibilityStory() {
        const [requiredVal, setRequiredVal] = useState('')
        ...
        return (
            ...
            >
                <section>
                    ...
                    >
                        Named search (aria-label) + required
                    </h3>
                    <SearchInputV2
                        ...
                        required
                        value={requiredVal}
                        onChange={(e) => setRequiredVal(e.target.value)}
...
- **required**: \`required\` and \`aria-required\` on the input
...
3. Use a screen reader to verify search field name, required state, and invalid state when \`error\` is true.
`,
            },
        },
        a11y: getA11yConfig('form'),
