import React from 'react'
import { describe, it, expect, vi } from 'vitest'
import { render, screen } from '../../test-utils'
import { axe } from 'jest-axe'
import MultiValueInput from '../../../lib/components/Inputs/MultiValueInput/MultiValueInput'
import { TextInputSize } from '../../../lib/components/Inputs/TextInput/types'

describe('MultiValueInput Accessibility', () => {
    describe('WCAG 2.1/2.2 Compliance (Level A, AA, AAA)', () => {
        it('meets WCAG standards for default multi-value input (axe-core validation)', async () => {
            const { container } = render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const results = await axe(container)
            expect(results).toHaveNoViolations()
        })

        it('meets WCAG standards for all input sizes (Small, Medium, Large)', async () => {
            const sizes = [
                TextInputSize.SMALL,
                TextInputSize.MEDIUM,
                TextInputSize.LARGE,
            ]

            for (const size of sizes) {
                const { container } = render(
                    <MultiValueInput
                        label={`${size} input`}
                        value=""
                        onChange={() => {}}
                        placeholder="Add tag..."
                        tags={[]}
                        onTagAdd={() => {}}
                        onTagRemove={() => {}}
                        size={size}
                    />
                )
                const results = await axe(container)
                expect(results).toHaveNoViolations()
            }
        })

        it('meets WCAG standards when disabled (2.1.1 Keyboard, 4.1.2 Name Role Value)', async () => {
            const { container } = render(
                <MultiValueInput
                    label="Disabled Input"
                    value=""
                    onChange={() => {}}
                    placeholder="This input is disabled"
                    tags={['readonly', 'tag']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    disabled
                />
            )
            const results = await axe(container)
            expect(results).toHaveNoViolations()
        })

        it('meets WCAG standards with error state (3.3.1 Error Identification)', async () => {
            const { container } = render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="At least one keyword is required"
                />
            )
            const results = await axe(container)
            expect(results).toHaveNoViolations()
        })

        it('meets WCAG standards with tags present (tag removal buttons)', async () => {
            const { container } = render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react', 'typescript', 'javascript']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const results = await axe(container)
            expect(results).toHaveNoViolations()
        })
    })

    describe('WCAG 3.3.2 Labels or Instructions (Level A)', () => {
        it('has accessible label associated with input - programmatically determinable', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    name="keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toBeInTheDocument()
            // Label association is handled via InputLabels component with htmlFor
        })

        it('renders sublabel for additional context - supplementary instructions', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    sublabel="Add relevant keywords for searchability"
                    name="keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            // Sublabel is rendered as "(Add relevant keywords for searchability)"
            expect(
                screen.getByText(/Add relevant keywords for searchability/)
            ).toBeInTheDocument()
        })

        it('renders hint text for additional guidance - instructions provided', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    hintText="Press Enter to add a keyword"
                    name="keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            expect(
                screen.getByText('Press Enter to add a keyword')
            ).toBeInTheDocument()
        })

        it('supports help icon with tooltip for detailed instructions', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    helpIconHintText="Keywords help categorize your content"
                    name="keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            // Help icon hint is rendered in InputLabels component
            expect(screen.getByRole('textbox')).toBeInTheDocument()
        })
    })

    describe('WCAG 3.3.1 Error Identification (Level A)', () => {
        it('displays error message when in error state - error identified in text', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="At least one keyword is required"
                />
            )
            expect(
                screen.getByText('At least one keyword is required')
            ).toBeInTheDocument()
        })

        it('applies error styling when error prop is true - error visually indicated', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="This field is required"
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toBeInTheDocument()
            // Error state is indicated via border color and shake animation
        })

        it('error message is associated with input - relationship programmatically determinable', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    name="keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="Invalid keywords format"
                />
            )
            expect(
                screen.getByText('Invalid keywords format')
            ).toBeInTheDocument()
            // Error association is handled via InputFooter component and aria-describedby
        })

        it('input has aria-invalid when error is true - error state programmatically determinable', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="Invalid input"
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toHaveAttribute('aria-invalid', 'true')
        })
    })

    describe('WCAG 3.3.3 Error Suggestion (Level AA)', () => {
        it('provides helpful error messages with correction suggestions', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="At least one keyword is required (e.g., add 'react' or 'javascript')"
                />
            )
            expect(
                screen.getByText(
                    "At least one keyword is required (e.g., add 'react' or 'javascript')"
                )
            ).toBeInTheDocument()
        })
    })

    describe('WCAG 1.3.5 Identify Input Purpose (Level AA - WCAG 2.1)', () => {
        it('supports name attribute for input purpose identification', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    name="keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toHaveAttribute('name', 'keywords')
        })

        it('supports id attribute for unique identification', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    id="custom-multi-value-input-id"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toHaveAttribute('id', 'custom-multi-value-input-id')
        })
    })

    describe('WCAG 2.1.1 Keyboard (Level A)', () => {
        it('is focusable with keyboard - all functionality operable via keyboard', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')

            input.focus()
            expect(document.activeElement).toBe(input)
        })

        it('can receive keyboard input - keyboard operable', async () => {
            const handleChange = vi.fn()
            const { user } = render(
                <MultiValueInput
                    label="Type Here"
                    value=""
                    onChange={handleChange}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )

            const input = screen.getByRole('textbox')
            await user.type(input, 'react')

            expect(handleChange).toHaveBeenCalled()
        })

        it('disabled input is not focusable - prevents keyboard interaction', () => {
            render(
                <MultiValueInput
                    label="Disabled"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    disabled
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toBeDisabled()
        })

        it('supports Tab key for focus navigation - logical focus order', async () => {
            const { user } = render(
                <>
                    <MultiValueInput
                        label="First"
                        value=""
                        onChange={() => {}}
                        placeholder="First input"
                        tags={[]}
                        onTagAdd={() => {}}
                        onTagRemove={() => {}}
                    />
                    <MultiValueInput
                        label="Second"
                        value=""
                        onChange={() => {}}
                        placeholder="Second input"
                        tags={[]}
                        onTagAdd={() => {}}
                        onTagRemove={() => {}}
                    />
                </>
            )

            const inputs = screen.getAllByRole('textbox')
            await user.tab()
            expect(document.activeElement).toBe(inputs[0])

            await user.tab()
            expect(document.activeElement).toBe(inputs[1])
        })

        it('supports Enter key to add tags - keyboard operable', async () => {
            const handleTagAdd = vi.fn()
            const { user } = render(
                <MultiValueInput
                    label="Add Tags"
                    value="react"
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={handleTagAdd}
                    onTagRemove={() => {}}
                />
            )

            const input = screen.getByRole('textbox')
            await user.type(input, '{Enter}')

            expect(handleTagAdd).toHaveBeenCalledWith('react')
        })

        it('supports Backspace key to remove last tag when input is empty - keyboard operable', async () => {
            const handleTagRemove = vi.fn()
            const { user } = render(
                <MultiValueInput
                    label="Remove Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react', 'typescript']}
                    onTagAdd={() => {}}
                    onTagRemove={handleTagRemove}
                />
            )

            const input = screen.getByRole('textbox')
            await user.type(input, '{Backspace}')

            expect(handleTagRemove).toHaveBeenCalledWith('typescript')
        })
    })

    describe('WCAG 2.4.7 Focus Visible (Level AA)', () => {
        it('shows focus indicator when focused - keyboard focus indicator visible', () => {
            render(
                <MultiValueInput
                    label="Focus Me"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')

            input.focus()
            expect(document.activeElement).toBe(input)
            // Focus indicator styling is applied via _focus prop (boxShadow)
        })

        it('maintains focus visible state during interaction', async () => {
            const { user } = render(
                <MultiValueInput
                    label="Type Here"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')

            await user.click(input)
            expect(document.activeElement).toBe(input)

            await user.type(input, 'test')
            expect(document.activeElement).toBe(input)
        })

        it('removes focus on blur - predictable focus behavior', () => {
            render(
                <MultiValueInput
                    label="Blur Test"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')

            input.focus()
            expect(document.activeElement).toBe(input)

            input.blur()
            expect(document.activeElement).not.toBe(input)
        })
    })

    describe('WCAG 2.4.3 Focus Order (Level A)', () => {
        it('maintains logical focus order - sequential navigation order', async () => {
            const { user } = render(
                <form>
                    <MultiValueInput
                        label="First"
                        value=""
                        onChange={() => {}}
                        placeholder="First input"
                        tags={[]}
                        onTagAdd={() => {}}
                        onTagRemove={() => {}}
                    />
                    <MultiValueInput
                        label="Second"
                        value=""
                        onChange={() => {}}
                        placeholder="Second input"
                        tags={[]}
                        onTagAdd={() => {}}
                        onTagRemove={() => {}}
                    />
                    <button type="submit">Submit</button>
                </form>
            )

            await user.tab()
            expect(document.activeElement).toBe(
                screen.getAllByRole('textbox')[0]
            )

            await user.tab()
            expect(document.activeElement).toBe(
                screen.getAllByRole('textbox')[1]
            )

            await user.tab()
            expect(document.activeElement).toBe(
                screen.getByRole('button', { name: 'Submit' })
            )
        })
    })

    describe('WCAG 4.1.2 Name, Role, Value (Level A)', () => {
        it('has proper textbox role - programmatically determinable role', () => {
            render(
                <MultiValueInput
                    label="Input Role"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            expect(screen.getByRole('textbox')).toBeInTheDocument()
        })

        it('announces label to screen readers - accessible name provided', () => {
            render(
                <MultiValueInput
                    label="Screen Reader Label"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            expect(screen.getByText('Screen Reader Label')).toBeInTheDocument()
        })

        it('exposes required state - state programmatically determinable', () => {
            render(
                <MultiValueInput
                    label="Required Field"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    required
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toHaveAttribute('required')
            expect(input).toHaveAttribute('aria-required', 'true')
        })

        it('announces disabled state - state programmatically determinable', () => {
            render(
                <MultiValueInput
                    label="Disabled Input"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    disabled
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toBeDisabled()
        })
    })

    describe('WCAG 2.5.8 Target Size (Minimum) - Level AA (WCAG 2.2)', () => {
        it('meets minimum touch target size requirements (24x24px for AA)', () => {
            render(
                <MultiValueInput
                    label="Touch Target"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toBeInTheDocument()
            // Input fields naturally exceed 24x24px minimum for Level AA
            // Actual size verification requires browser DevTools measurement
        })
    })

    describe('Tag Removal Button Accessibility (WCAG 2.1.1, 2.5.8)', () => {
        it('tag remove buttons have accessible names - aria-label provided', () => {
            render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react', 'typescript']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )

            expect(screen.getByLabelText('Remove react')).toBeInTheDocument()
            expect(
                screen.getByLabelText('Remove typescript')
            ).toBeInTheDocument()
        })

        it('tag remove buttons are keyboard accessible - Enter key activates', async () => {
            const handleTagRemove = vi.fn()
            const { user } = render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react']}
                    onTagAdd={() => {}}
                    onTagRemove={handleTagRemove}
                />
            )

            const removeButton = screen.getByLabelText('Remove react')
            await user.click(removeButton)

            expect(handleTagRemove).toHaveBeenCalledWith('react')
        })

        it('tag remove buttons are keyboard accessible - Space key activates', async () => {
            const handleTagRemove = vi.fn()
            const { user } = render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react']}
                    onTagAdd={() => {}}
                    onTagRemove={handleTagRemove}
                />
            )

            const removeButton = screen.getByLabelText('Remove react')
            removeButton.focus()
            await user.keyboard(' ')

            expect(handleTagRemove).toHaveBeenCalledWith('react')
        })

        it('tag remove buttons meet minimum touch target size (24x24px)', () => {
            render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )

            const removeButton = screen.getByLabelText('Remove react')
            expect(removeButton).toBeInTheDocument()
            // Touch target size is set via minWidth and minHeight styles (24px)
            // Actual size verification requires browser DevTools measurement
        })

        it('tag remove buttons have type="button" to prevent form submission', () => {
            render(
                <form>
                    <MultiValueInput
                        label="Tags"
                        value=""
                        onChange={() => {}}
                        placeholder="Add tag..."
                        tags={['react']}
                        onTagAdd={() => {}}
                        onTagRemove={() => {}}
                    />
                </form>
            )

            const removeButton = screen.getByLabelText('Remove react')
            expect(removeButton).toHaveAttribute('type', 'button')
        })
    })

    describe('Form Integration & Required Fields', () => {
        it('displays required indicator when required prop is true (3.3.2)', () => {
            render(
                <MultiValueInput
                    label="Required Field"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    required
                />
            )
            // Required indicator (asterisk) is rendered in InputLabels component
            const input = screen.getByRole('textbox')
            expect(input).toHaveAttribute('required')
            expect(input).toHaveAttribute('aria-required', 'true')
        })

        it('works correctly in form submission context', () => {
            const handleSubmit = vi.fn((e) => e.preventDefault())
            render(
                <form onSubmit={handleSubmit}>
                    <MultiValueInput
                        label="Keywords"
                        name="keywords"
                        value=""
                        onChange={() => {}}
                        placeholder="Add keyword..."
                        tags={[]}
                        onTagAdd={() => {}}
                        onTagRemove={() => {}}
                        required
                    />
                    <button type="submit">Submit</button>
                </form>
            )

            const input = screen.getByRole('textbox')
            expect(input).toHaveAttribute('name', 'keywords')
            expect(input).toHaveAttribute('required')
        })

        it('supports form validation with required and error states', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    name="keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    required
                    error
                    errorMessage="At least one keyword is required"
                />
            )

            const input = screen.getByRole('textbox')
            expect(input).toHaveAttribute('required')
            expect(input).toHaveAttribute('aria-invalid', 'true')
            expect(
                screen.getByText('At least one keyword is required')
            ).toBeInTheDocument()
        })
    })

    describe('Focus and Blur Events (WCAG 3.2.1 On Focus - Level A)', () => {
        it('calls onFocus when input receives focus - predictable behavior', () => {
            const handleFocus = vi.fn()
            render(
                <MultiValueInput
                    label="Focus Event"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    onFocus={handleFocus}
                />
            )

            const input = screen.getByRole('textbox')
            input.focus()

            expect(handleFocus).toHaveBeenCalledTimes(1)
        })

        it('calls onBlur when input loses focus - predictable behavior', () => {
            const handleBlur = vi.fn()
            render(
                <MultiValueInput
                    label="Blur Event"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    onBlur={handleBlur}
                />
            )

            const input = screen.getByRole('textbox')
            input.focus()
            input.blur()

            expect(handleBlur).toHaveBeenCalledTimes(1)
        })

        it('does not change context unexpectedly on focus (3.2.1)', () => {
            render(
                <MultiValueInput
                    label="No Context Change"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    onFocus={() => {
                        // Focus should not trigger unexpected context changes
                        // like form submission or navigation
                    }}
                />
            )

            const input = screen.getByRole('textbox')
            input.focus()

            expect(document.activeElement).toBe(input)
        })
    })

    describe('Placeholder Text (WCAG 3.3.2 - Best Practice)', () => {
        it('provides placeholder text for additional guidance - not a label replacement', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    placeholder="Type and press Enter to add..."
                    value=""
                    onChange={() => {}}
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            const input = screen.getByRole('textbox')
            expect(input).toHaveAttribute(
                'placeholder',
                'Type and press Enter to add...'
            )
            // Note: Placeholder is supplementary to label, not a replacement
        })

        it('label is always present even with placeholder - proper labeling', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    placeholder="Add keyword..."
                    value=""
                    onChange={() => {}}
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )
            expect(screen.getByText('Keywords')).toBeInTheDocument()
            expect(screen.getByRole('textbox')).toHaveAttribute(
                'placeholder',
                'Add keyword...'
            )
        })
    })

    describe('ARIA DescribedBy (WCAG 4.1.2 Name, Role, Value - Level A)', () => {
        it('associates hint text with input via aria-describedby', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    hintText="Press Enter to add a keyword"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )

            const input = screen.getByRole('textbox')
            const hintId = input.getAttribute('aria-describedby')?.split(' ')[0]

            expect(hintId).toBeTruthy()
            expect(
                screen.getByText('Press Enter to add a keyword')
            ).toHaveAttribute('id', hintId)
        })

        it('associates error message with input via aria-describedby', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="At least one keyword is required"
                />
            )

            const input = screen.getByRole('textbox')
            const errorId = input
                .getAttribute('aria-describedby')
                ?.split(' ')[0]

            expect(errorId).toBeTruthy()
            expect(
                screen.getByText('At least one keyword is required')
            ).toHaveAttribute('id', errorId)
        })

        it('error message has role="alert" for screen reader announcements', () => {
            render(
                <MultiValueInput
                    label="Keywords"
                    value=""
                    onChange={() => {}}
                    placeholder="Add keyword..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="Invalid input"
                />
            )

            const errorMessage = screen.getByText('Invalid input')
            expect(errorMessage).toHaveAttribute('role', 'alert')
            expect(errorMessage).toHaveAttribute('aria-live', 'polite')
        })
    })

    describe('Tag Management Accessibility', () => {
        it('tags are displayed with accessible text content', () => {
            render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react', 'typescript']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )

            expect(screen.getByText('react')).toBeInTheDocument()
            expect(screen.getByText('typescript')).toBeInTheDocument()
        })

        it('tag removal maintains focus after removal', async () => {
            const handleTagRemove = vi.fn()
            const { user } = render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react']}
                    onTagAdd={() => {}}
                    onTagRemove={handleTagRemove}
                />
            )

            const removeButton = screen.getByLabelText('Remove react')

            await user.click(removeButton)

            // Focus should return to input after tag removal
            expect(handleTagRemove).toHaveBeenCalled()
        })

        it('supports multiple tags with individual remove buttons', () => {
            render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react', 'typescript', 'javascript']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                />
            )

            expect(screen.getByLabelText('Remove react')).toBeInTheDocument()
            expect(
                screen.getByLabelText('Remove typescript')
            ).toBeInTheDocument()
            expect(
                screen.getByLabelText('Remove javascript')
            ).toBeInTheDocument()
        })
    })

    describe('Comprehensive WCAG 2.1/2.2 Compliance (Level A, AA, AAA)', () => {
        it('meets WCAG standards with all features combined - comprehensive test', async () => {
            const { container } = render(
                <MultiValueInput
                    label="Complete Test"
                    sublabel="Additional context"
                    hintText="Helpful hint"
                    helpIconHintText="Tooltip information"
                    placeholder="Add value..."
                    value=""
                    onChange={() => {}}
                    tags={['tag1', 'tag2']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    required
                />
            )
            const results = await axe(container)
            expect(results).toHaveNoViolations()
            // Tests: 1.3.5, 2.1.1, 2.4.7, 3.3.2, 4.1.2
        })

        it('meets WCAG standards with error state - all error requirements', async () => {
            const { container } = render(
                <MultiValueInput
                    label="Error Test"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={[]}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    error
                    errorMessage="Please correct this field"
                    required
                />
            )
            const results = await axe(container)
            expect(results).toHaveNoViolations()
            // Tests: 3.3.1 Error Identification, 3.3.3 Error Suggestion, 4.1.2
        })

        it('meets WCAG standards in disabled state - all disabled state requirements', async () => {
            const { container } = render(
                <MultiValueInput
                    label="Disabled"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['readonly', 'tag']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    disabled
                />
            )
            const results = await axe(container)
            expect(results).toHaveNoViolations()
            // Tests: 2.1.1 Keyboard, 4.1.2 Name Role Value
        })

        it('meets WCAG standards with tags and remove buttons - all tag interaction requirements', async () => {
            const { container } = render(
                <MultiValueInput
                    label="Tags"
                    value=""
                    onChange={() => {}}
                    placeholder="Add tag..."
                    tags={['react', 'typescript', 'javascript']}
                    onTagAdd={() => {}}
                    onTagRemove={() => {}}
                    required
                />
            )
            const results = await axe(container)
            expect(results).toHaveNoViolations()
            // Tests: 2.1.1 Keyboard, 2.5.8 Target Size, 4.1.2 Name Role Value
        })
    })
})
