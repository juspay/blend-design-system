...
- **MenuV2** – Refactored API with **token-driven styling** (no hardcoded colors/z-index/dimensions), **grouped virtual config** (`virtualScrolling?: { itemHeight?, overscan?, threshold? }`), **slots type** (`MenuV2ItemSlots`: `leading` + `trailing[]` with backward-compatible `slot1`–`slot4`), optional `id` on groups and items; correct `collisionBoundaryRef`; TanStack Virtual for large lists; theme via `getMenuV2Tokens(foundationToken, theme)` from `menuV2.tokens`.
...
- **Item content**: Label, optional subLabel, up to 4 slots (v1: `slot1`–`slot4`; V2: `slots?: { leading?, trailing? }` or legacy `slot1`–`slot4`), variant (default/action), actionType (primary/danger), disabled, onClick, tooltip/tooltipProps.
- **Submenus**: Nested items; optional `enableSubMenuSearch`, `subMenuSearchPlaceholder`; V2 supports `enableSubMenuVirtualScrolling`, `subMenuVirtualItemHeight`, `subMenuVirtualOverscan`.
...
- **State**: `open`, `onOpenChange` for controlled open; `asModal`; optional `skeleton`.
...
- **Slots**: Adds **`slots?: MenuV2ItemSlots`** (`{ leading?, trailing? }`) with `slot1`–`slot4` retained but deprecated; `getItemSlots(item)` in utils resolves either API.
...
| skeleton                                                                              | MenuSkeletonProps                 | MenuV2SkeletonProps                   |
...
| Item: label, subLabel, slot1–slot4                                                    | ✓                                 | ✓ + slots?: MenuV2ItemSlots           |
...
| Item: enableSubMenuVirtualScrolling, subMenuVirtualItemHeight, subMenuVirtualOverscan | ✓ + subMenuVirtualScrollThreshold | ✓ (no subMenuVirtualScrollThreshold)  |
...
- **subMenuVirtualScrollThreshold** on item type. V2 omits this on submenu config.
- **collisonBoundaryRef** (typo) – same behavior as V2’s `collisionBoundaryRef`.

### V2-only

- **slots**: `MenuV2ItemSlots` (`leading`, `trailing[]`) for clearer slot API; `slot1`–`slot4` still supported.
- **virtualScrolling**: Single config object `{ itemHeight?, overscan?, threshold? }`.
- **id** on `MenuV2GroupType` and `MenuV2ItemType`.
- **Token export**: `getMenuV2Tokens` is exported from **`menuV2.tokens`** (not from light/dark files); theme is chosen inside the getter.
...
- **V2**: `MenuV2ItemType`, `MenuV2GroupType` – optional `id`; `slots?: MenuV2ItemSlots` with deprecated `slot1`–`slot4`; same shape otherwise (label, subLabel, variant, actionType, disabled, onClick, subMenu, search, tooltip, submenu virtual options).

### Internal / utils (V2)

- **getItemSlots(item)**: Returns `[slot1, slot2, slot3, slot4]` from `item.slots` or legacy `item.slot1`–`slot4`.
...
- **content**: backgroundColor, border, borderRadius, boxShadow, **zIndex**, **minWidth**, **maxWidth**, padding (top/right/bottom/left). Used for main menu and submenu panel; no hardcoded z-index or dimensions in components.
...
### 4. Slots: MenuV2ItemSlots + legacy slot1–slot4

**Decision**: V2 introduces `slots?: { leading?, trailing? }` with deprecated `slot1`–`slot4`. `getItemSlots(item)` normalizes both so rendering logic is single path.

**Rationale**: Clearer API for new code; backward compatibility without duplicating render branches.
