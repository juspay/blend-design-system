...
<DocsTypeTable
    data={[
        ...
        [
            {
                content: 'DrawerContent.isContentPersistent',
                hintText:
                    'When true, keeps the drawer content mounted in the DOM even when closed. Useful for preserving in-flight UI state such as chat widgets, form inputs, or API-driven panels. When false, content is unmounted when drawer closes. Defaults to false.',
            },
            {
                content: 'boolean',
                hintText:
                    'true keeps content mounted when closed, false unmounts content on close',
            },
            { content: '' },
        ],
        [
