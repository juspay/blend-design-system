...
function getAllComponents() {
    ...
    return entries
        ...
        .filter(
            (name) => !EXISTING_META_COMPONENTS.includes(name.toLowerCase())
        )
}
    ...
    const propsArray = props
        .map((prop) => {
            const typeDefinition =
                enums.find((e) => e.name === prop.type)?.definition ||
...
async function generateMetaFiles() {
    ...
    console.log(
        `Found ${components.length} components without meta files:`,
        components
    )
    ...
    for (const componentName of components) {
        ...
        const metaContent = generateMetaFileContent(
            componentName,
            extractedData
        )
