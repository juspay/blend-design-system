...
function buildSections(sizeTokens: Record<string, unknown>): Section[] {
    ...
    for (const [key, value] of Object.entries(sizeTokens)) {
        if (isLeafValue(value)) {
            primitives.push({ key, defaultOpen: DEFAULT_OPEN_KEYS.has(key) })
        } else if (isObject(value)) {
            if (PRIORITY_KEYS.has(key)) {
                containers.push({ key, defaultOpen: true })
            } else {
                rest.push({ key, defaultOpen: DEFAULT_OPEN_KEYS.has(key) })
            }
        }
    }
...
export function ComponentTokenEditor({
    ...
    const overriddenCount = useMemo(
        () => collectLeafPaths(tokenOverrides).length,
        [tokenOverrides]
    )
    ...
    return (
        <div className="space-y-3">
            <p className="text-xs text-gray-400">
                Edit tokens for this component. Save &rarr; Publish &rarr; CLI
                pull to use in your app.
            </p>

            {overriddenCount > 0 && (
                <div className="flex items-center justify-between px-3 py-2 bg-blue-50 rounded-lg text-xs">
                    <span className="text-blue-700 font-medium">
                        {overriddenCount} token
                        {overriddenCount !== 1 ? 's' : ''} overridden
                    </span>
                    <button
                        onClick={() => {
                            for (const p of collectLeafPaths(tokenOverrides))
                                onRemove(p)
                        }}
