...
export default function RelatedCard({ item }: RelatedCardProps) {
    return (
        <div>
            ...
            <Link
                href={`/showcase/${encodeURIComponent(item.id)}`}
                className="group shrink-0 text-left px-7 py-8 border border-border rounded-[0_12.77px_12.773px_12.773px] bg-secondary/40 hover:bg-secondary/65 transition-colors duration-100 block"
            >
