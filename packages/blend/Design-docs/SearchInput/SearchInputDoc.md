...
Declared in `SearchInputV2.types.ts`; HTML attributes from **`Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size' | 'style' | 'className' | 'placeholderStyles' | 'required'>`**.
...
**Notable inherited props** (non-exhaustive): **`id`**, **`name`**, **`placeholder`**, **`autoComplete`**, **`aria-label`**, **`aria-labelledby`**, **`data-*`**. **`className`** / **`style`** are omitted from the type; **`filterBlockedProps`** strips **`className`** and **`style`** from spreadable rest.
...
**Decision**: **`onFocus`** / **`onBlur`** are destructured and merged so internal focus state updates run; **`{...filteredRest}`** is spread on **`PrimitiveInput`** after those handlers so consumer **`id`**, **`aria-*`**, etc. still apply.
